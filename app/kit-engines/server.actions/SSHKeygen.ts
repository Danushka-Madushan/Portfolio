...
export class SSHKeygen {
   ...
   */
  private static convertToSSHFormat = (
    keyObject: KeyObject,
    isPrivate: boolean,
    passphrase?: string,
    comment?: string
   ...
   */
  private static calculateFingerprint = (publicKeySSH: string): string => {
    try {
  ...
  * ```
  */
  public static generateEd25519Keys = async (options: Ed25519Options = {}): Promise<SSHKeyPair> => {
    ...
    const publicKeySSH = this.convertToSSHFormat(publicKey, false, undefined, comment);
    const privateKeySSH = this.convertToSSHFormat(privateKey, true, passphrase);

    // Calculate fingerprint
    const fingerprint = this.calculateFingerprint(publicKeySSH);
   ...
   * ```
   */
  public static generateRSAKeys = async (options: RSAOptions): Promise<SSHKeyPair> => {
    try {
   ...
   * ```
   */
  public static generateECDSAKeys = async (options: ECDSAOptions): Promise<SSHKeyPair> => {
    try {
